...
const freemaninit = (function() {
    ...
    const portofolio = function(e) {
        if (!porto || !btns || !btns.length || !Shuffle) return;
        
        // Find the initial active group based on the button that has 'active' class
        var activeBtn = btnContainer.querySelector('.active');
        var initialGroup = Shuffle.ALL_ITEMS;
        if (activeBtn) {
            var groupAttr = activeBtn.getAttribute('data-group');
            if (groupAttr && groupAttr !== 'all') {
                initialGroup = groupAttr;
            }
        }
        ...
        for (var i = 0; i < btns.length; i++) {
            btns[i].addEventListener("click", function(e) {
                // target only the active element under our filter container
                var activeElement = btnContainer.querySelector('.active');
                if (activeElement) {
    ...
    const scrolspy = function(e) {
        if (!menuSection || !mainSection) return;
        // for clickable event
    ...
    const buttonclick = function(e) {
        ...
        if (mobilenav) {
            mobilenav.addEventListener("click", function(e) {
                ...
                if (body) body.classList.toggle('openmenu');
            }, false);
        }
        ...
        // Theme Toggle
        var themeToggleBtn = document.querySelector('.btn-theme-toggle');
        if (themeToggleBtn) {
